...
function doGet(e) {
  try {
    ...
    if(action==='public') result={success:true,changes:cmsList_(String(p.page||'')),items:cmsItems_(String(p.page||''))};
      ...
      else if(action==='notification-subscribers') result=qmsNotificationSubscribers_();
...
function doPost(e) {
  try {
    ...
    else if(p.action==='send-notification-broadcast') result=qmsSendNotificationBroadcast_(p);
    else throw new Error('Invalid request.');
...
function qmsNotificationSheet_(){
  const headers=['UPDATED_AT','REFERENCE','PARTICIPANT','EMAIL','CONTACT_NUMBER','ACTIVITY_SOURCE','EMAIL_CONSENT','SMS_CONSENT','STATUS'];
  const ss=cmsSpreadsheet_();let sh=ss.getSheetByName('FUTURE_ACTIVITY_CONSENTS');
  if(!sh){sh=ss.insertSheet('FUTURE_ACTIVITY_CONSENTS');sh.getRange(1,1,1,headers.length).setValues([headers]);sh.setFrozenRows(1);}
  return sh;
}
function qmsSaveNotificationConsent_(p){
  if(!p.emailConsent&&!p.smsConsent)return;
  const sh=qmsNotificationSheet_();
  sh.appendRow([new Date(),p.reference||'',p.participant||'',p.email||'',p.contact||'',p.activity||'',p.emailConsent?'YES':'NO',p.smsConsent?'YES':'NO','ACTIVE']);
}
function qmsNotificationSubscribers_(){
  const sh=qmsNotificationSheet_();if(sh.getLastRow()<2)return {success:true,items:[]};
  const rows=sh.getRange(2,1,sh.getLastRow()-1,9).getDisplayValues();
  return {success:true,items:rows.filter(r=>String(r[8]||'ACTIVE').toUpperCase()!=='UNSUBSCRIBED').map(r=>({updatedAt:r[0],reference:r[1],participant:r[2],email:r[3],contact:r[4],activity:r[5],emailConsent:String(r[6]).toUpperCase()==='YES',smsConsent:String(r[7]).toUpperCase()==='YES',status:r[8]||'ACTIVE'}))};
}
function qmsSendNotificationBroadcast_(p){
  if(!cmsAuthorized_(p.password))throw new Error('Incorrect administrator password.');
  const channel=String(p.channel||'email').toLowerCase(),group=String(p.group||'all'),activity=String(p.activity||''),subject=String(p.subject||'SK Sapilang Activity Update').trim(),message=String(p.message||'').trim();
  if(!message)throw new Error('Announcement message is required.');
  let items=qmsNotificationSubscribers_().items||[];
  if(group==='activity')items=items.filter(x=>x.activity===activity);
  const seen={};items=items.filter(x=>{const key=channel==='sms'?String(x.contact||'').replace(/\D/g,''):String(x.email||'').toLowerCase();if(!key||seen[key])return false;seen[key]=1;return channel==='sms'?x.smsConsent:x.emailConsent;});
  let sent=0,failed=0,skipped=0;const errors=[];
  if(channel==='email'){
    items.forEach(x=>{try{MailApp.sendEmail({to:x.email,subject:subject||'SK Sapilang Activity Update',htmlBody:'<p>Good day'+(x.participant?', <strong>'+cmsEscapeHtml_(x.participant)+'</strong>':'')+'.</p><p>'+cmsEscapeHtml_(message).replace(/\n/g,'<br>')+'</p><p><small>You are receiving this because you consented to future SK Sapilang activity announcements by email.</small></p><p>Sangguniang Kabataan of Barangay Sapilang</p>'});sent++;}catch(e){failed++;errors.push(String(e.message||e).slice(0,100));}});
    return {success:true,sent:sent,failed:failed,skipped:skipped,note:items.length?'Email broadcast completed.':'No consented email recipients matched this group.'};
  }
  if(channel==='sms'){
    const props=PropertiesService.getScriptProperties();const url=String(props.getProperty('SMS_GATEWAY_URL')||'').trim(),token=String(props.getProperty('SMS_GATEWAY_TOKEN')||'').trim();
    if(!url)throw new Error('SMS gateway is not configured. Set SMS_GATEWAY_URL (and SMS_GATEWAY_TOKEN if required) in Apps Script Script Properties first.');
    items.forEach(x=>{try{const payload={to:x.contact,message:message};const opt={method:'post',contentType:'application/json',payload:JSON.stringify(payload),muteHttpExceptions:true,headers:token?{Authorization:'Bearer '+token}:{}};const r=UrlFetchApp.fetch(url,opt);const code=r.getResponseCode();if(code>=200&&code<300)sent++;else{failed++;errors.push('HTTP '+code);}}catch(e){failed++;errors.push(String(e.message||e).slice(0,100));}});
    return {success:true,sent:sent,failed:failed,skipped:skipped,note:items.length?'SMS gateway requests completed.':'No consented SMS recipients matched this group.'};
  }
...
function qmsSaveActivity_(p,parameters){
  const required=['activity','participant','programFormat','certificatePreference','rating'];
  ...
  const futureEmailConsent=String(p.futureEmailConsent||'').toLowerCase()==='yes';
  const futureSmsConsent=String(p.futureSmsConsent||'').toLowerCase()==='yes';
  if(futureEmailConsent&&!email)throw new Error('Enter an email address if you want future announcements by email.');
  if(futureSmsConsent&&!contact)throw new Error('Enter a mobile number if you want future announcements by text/SMS.');
  ...
  // IMPORTANT: the certificate recipient name comes directly from the Full Name
  // entered in the Activity Evaluation form (p.participant). No second name entry is needed.
  const cert=cmsIssueCertificate_({
    participant:String(p.participant||'').trim(),
    email:email,
    activity:p.activity,
    activityType:p.programFormat||p.activityType,
    date:p.date,
    venue:p.venue,
    speaker:p.speaker,
    qmsReference:reference,
    certificatePreference:p.certificatePreference||'Digital Certificate'
  });

  // Send the participant a copy of the non-sensitive evaluation responses.
  const feedbackCopy=cmsSendActivityFeedbackCopy_({
    participant:String(p.participant||'').trim(),
    email:email,
    activity:String(p.activity||''),
    reference:reference,
    certificateId:cert.certificateId,
    rating:p.rating, relevance:p.relevance, objectives:p.objectives,
    facilitatorRating:p.facilitatorRating, organization:p.organization,
    venueRating:p.venueRating, materials:p.materials,
    timeManagement:p.timeManagement, engagement:p.engagement,
    learning:p.learning, likedMost:p.likedMost, improvement:p.improvement,
    future:p.future
  });
  cmsSetFeedbackCopyStatus_(cert.certificateId,feedbackCopy.status);

  return {success:true,reference:reference,score:avg,rating:cmsQualityLabel_(avg),certificateId:cert.certificateId,emailSent:cert.emailSent,emailStatus:cert.emailStatus,feedbackCopyStatus:feedbackCopy.status,certificatePreference:cert.certificatePreference,hardCopyAvailableOn:cert.hardCopyAvailableOn};
}
...
function cmsCertificatesSheet_(){
  const headers=['CERTIFICATE_ID','QMS_REFERENCE','PARTICIPANT','ACTIVITY','ACTIVITY_TYPE','DATE_CONDUCTED','VENUE','RESOURCE_SPEAKER','ISSUED_AT','STATUS','EMAIL','EMAIL_STATUS','DELIVERY_PREFERENCE','HARD_COPY_AVAILABLE_ON','PRINT_STATUS','FEEDBACK_COPY_STATUS'];
...
function cmsIssueCertificate_(p){
  ...
  if(sheet.getLastRow()>1){const rows=sheet.getRange(2,1,sheet.getLastRow()-1,16).getDisplayValues();const ex=rows.find(r=>r[1]===qmsReference);if(ex)return {success:true,certificateId:ex[0],existing:true,emailSent:/SENT/i.test(ex[11]||''),certificatePreference:ex[12]||'Digital Certificate',hardCopyAvailableOn:ex[13]||''};}
  ...
  sheet.appendRow([id,qmsReference,participant,activity,String(p.activityType||''),String(p.date||''),String(p.venue||''),String(p.speaker||''),new Date(),'ACTIVE',email,emailStatus,preference,hardCopyAvailableOn,hardCopyRequested?'PRINT REQUESTED':'DIGITAL ONLY','PENDING']);
...
function cmsSetFeedbackCopyStatus_(certificateId,status){
  if(!certificateId)return;
  const sh=cmsCertificatesSheet_();
  if(sh.getLastRow()<2)return;
  const ids=sh.getRange(2,1,sh.getLastRow()-1,1).getDisplayValues().flat();
  const i=ids.indexOf(String(certificateId));
  if(i>=0)sh.getRange(i+2,16).setValue(String(status||''));
}
function cmsSendActivityFeedbackCopy_(p){
  const email=String(p.email||'').trim();
  if(!email)return {sent:false,status:'NOT PROVIDED'};
  try{
    const esc=cmsEscapeHtml_;
    const rows=[
      ['Overall Rating',p.rating],['Relevance',p.relevance],['Objectives',p.objectives],
      ['Facilitator / Program Delivery',p.facilitatorRating],['Organization',p.organization],
      ['Venue',p.venueRating],['Materials',p.materials],['Time Management',p.timeManagement],
      ['Engagement',p.engagement],['Key Learning',p.learning],['Liked Most',p.likedMost],
      ['Suggested Improvement',p.improvement],['Future Activities / Suggestions',p.future]
    ];
    const body=rows.filter(function(r){return String(r[1]||'').trim();}).map(function(r){
      return '<tr><td style="padding:7px;border:1px solid #ddd"><strong>'+esc(r[0])+'</strong></td><td style="padding:7px;border:1px solid #ddd">'+esc(r[1])+'</td></tr>';
    }).join('');
    const certUrl='https://sk-sapilang.github.io/sk-sapilang-website/certificate.html?id='+encodeURIComponent(String(p.certificateId||''));
    const adminCopy=(cmsCertificateSettings_().settings||{}).adminEmail||'';
    MailApp.sendEmail({
      to:email,
      bcc:adminCopy||undefined,
      subject:'Copy of Your SK Sapilang Activity Evaluation – '+String(p.activity||''),
      htmlBody:'<p>Good day, <strong>'+esc(p.participant)+'</strong>.</p>'+
        '<p>Thank you for completing the evaluation for <strong>'+esc(p.activity)+'</strong>. Below is a copy of the responses you submitted.</p>'+
        '<p>QMS Reference: <strong>'+esc(p.reference)+'</strong><br>Certificate No.: <strong>'+esc(p.certificateId)+'</strong></p>'+
        '<table style="border-collapse:collapse;width:100%;max-width:700px">'+body+'</table>'+
        '<p><a href="'+certUrl+'">View / Print Your Digital Certificate</a></p>'+
        '<p><small>Optional demographic/GAD monitoring responses are not included in this email copy.</small></p>'+
        '<p>Sangguniang Kabataan of Barangay Sapilang</p>'
    });
    return {sent:true,status:'SENT'};
  }catch(err){
    return {sent:false,status:'FAILED: '+String(err&&err.message?err.message:err).slice(0,120)};
  }
}
...
function cmsCertificateList_() {
  var sh = cmsCertificatesSheet_();
  if (sh.getLastRow() < 2) {
    return { success: true, certificates: [] };
  }
  var rows = sh.getRange(2, 1, sh.getLastRow() - 1, 16).getDisplayValues().reverse();
  var certificates = rows.map(function (r) {
    return {
      certificateId: r[0],
      qmsReference: r[1],
      participant: r[2],
      ...
      email: r[10],
      emailStatus: r[11],
      deliveryPreference: r[12] || 'Digital Certificate',
      hardCopyAvailableOn: r[13],
      printStatus: r[14] || '',
      feedbackCopyStatus: r[15] || ''
    };
  });
...
function cmsUpdateCertificateStatus_(p) {
  if (!cmsAuthorized_(p.adminKey || p.password)) {
    throw new Error('Incorrect administrator password.');
  }
  var id = String(p.certificateId || '').trim().toUpperCase();
  var sh = cmsCertificatesSheet_();
  if (sh.getLastRow() < 2) throw new Error('Certificate not found.');
  var ids = sh.getRange(2, 1, sh.getLastRow() - 1, 1).getDisplayValues().flat();
...
function qmsRecycleSheet_() {
  var headers = ['DELETED_AT', 'RECORD_TYPE', 'REFERENCE', 'DATA_JSON'];
  var ss = cmsSpreadsheet_();
  var sh = ss.getSheetByName('QMS_RECYCLE_BIN');
  if (!sh) {
    sh = ss.insertSheet('QMS_RECYCLE_BIN');
...
function cmsCertificateSettingsSheet_() {
  var ss = cmsSpreadsheet_();
  var sh = ss.getSheetByName('CERTIFICATE_SETTINGS');
  if (!sh) {
    sh = ss.insertSheet('CERTIFICATE_SETTINGS');
    sh.appendRow(['KEY', 'VALUE']);
    var defaults = [
      ['title', 'Certificate of Participation'],
      ['body', 'for successfully participating in'],
      ['logo1', 'images/sk-logo.svg'],
      ['logo2', 'images/barangay-logo.svg'],
      ['signatory1', 'DANDY F. NILLO'],
      ['position1', 'SK Chairperson'],
      ['signatory2', 'SANGGUNIANG KABATAAN'],
      ['position2', 'Barangay Sapilang'],
      ['design', 'auto'],
      ['adminEmail', '']
    ];
...
function cmsCertificateSettings_() {
  var values = cmsCertificateSettingsSheet_().getDataRange().getDisplayValues().slice(1);
  var settings = {};
  values.forEach(function (r) { settings[r[0]] = r[1]; });
  return { success: true, settings: settings };
}

function cmsSaveCertificateSettings_(p) {
  var sh = cmsCertificateSettingsSheet_();
  var keys = [
    'title', 'body', 'logo1', 'logo2', 'signatory1', 'position1',
    'signatory2', 'position2', 'design', 'adminEmail',
    'certificateBackground', 'canvaEditUrl',
    'nameX', 'nameY', 'nameSize', 'qrX', 'qrY', 'qrSize',
    'noX', 'noY', 'noSize'
  ];
  var vals = sh.getDataRange().getDisplayValues();
  keys.forEach(function (k) {
    if (p[k] === undefined) return;
    var i = vals.findIndex(function (r, n) { return n > 0 && r[0] === k; });
    if (i > 0) {
      sh.getRange(i + 1, 2).setValue(String(p[k] || ''));
    } else {
      sh.appendRow([k, String(p[k] || '')]);
      vals.push([k, String(p[k] || '')]);
    }
  });
  return { success: true };
}
...
function qmsMoveToRecycle_(type, reference) {
  var map = {
    activity: 'ACTIVITY_EVALUATIONS',
    client: 'VISITOR_LOGBOOK',
    suggestion: 'SUGGESTIONS_RECOMMENDATIONS'
  };
  var name = map[type];
  if (!name) throw new Error('Invalid QMS record type.');
  var sh = cmsSpreadsheet_().getSheetByName(name);
  if (!sh || sh.getLastRow() < 2) throw new Error('Record not found.');
  var vals = sh.getDataRange().getDisplayValues();
  var headers = vals[0];
  var refCol = headers.indexOf('REFERENCE');
  var i = vals.findIndex(function (r, n) { return n > 0 && r[refCol] === reference; });
  if (i < 1) throw new Error('Record not found.');
  var obj = {};
  headers.forEach(function (h, j) { obj[h] = vals[i][j]; });
  qmsRecycleSheet_().appendRow([new Date(), type, reference, JSON.stringify(obj)]);
  sh.deleteRow(i + 1);
  return { success: true };
}

function qmsRecycleList_() {
  var sh = qmsRecycleSheet_();
  if (sh.getLastRow() < 2) return { success: true, records: [] };
  var rows = sh.getRange(2, 1, sh.getLastRow() - 1, 4).getDisplayValues().reverse();
  return {
    success: true,
    records: rows.map(function (r) {
      return { deletedAt: r[0], type: r[1], reference: r[2], data: r[3] };
    })
  };
}

function qmsRestore_(reference) {
  var bin = qmsRecycleSheet_();
  if (bin.getLastRow() < 2) throw new Error('Recycle record not found.');
  var vals = bin.getRange(2, 1, bin.getLastRow() - 1, 4).getDisplayValues();
  var i = vals.findIndex(function (r) { return r[2] === reference; });
  if (i < 0) throw new Error('Recycle record not found.');
  var type = vals[i][1];
  var obj = JSON.parse(vals[i][3]);
  var map = {
    activity: 'ACTIVITY_EVALUATIONS',
    client: 'VISITOR_LOGBOOK',
    suggestion: 'SUGGESTIONS_RECOMMENDATIONS'
  };
  var sh = cmsSpreadsheet_().getSheetByName(map[type]);
  if (!sh) throw new Error('Destination sheet not found.');
...
function qmsBackupToDrive_() {
  ...
  var folderId = props.getProperty('QMS_BACKUP_FOLDER_ID');
  var folder = null;
  try {
  ...
  if (!folder) {
    folder = DriveApp.createFolder('SK Sapilang QMS Backups');
    props.setProperty('QMS_BACKUP_FOLDER_ID', folder.getId());
  }
  ...
  var copy = DriveApp.getFileById(ss.getId()).makeCopy('SK Sapilang QMS Backup ' + stamp, folder);
  return { success: true, name: copy.getName(), url: copy.getUrl() };
}

function qmsResendCertificate_(id) {
  var sh = cmsCertificatesSheet_();
  var vals = sh.getDataRange().getDisplayValues();
  var i = vals.findIndex(function (r, n) { return n > 0 && r[0] === id; });
  if (i < 1) throw new Error('Certificate not found.');
  var r = vals[i];
  var email = String(r[10] || '').trim();
  if (!email) throw new Error('This participant did not provide an email address.');
  var url = 'https://sk-sapilang.github.io/sk-sapilang-website/certificate.html?id=' + encodeURIComponent(id);
  var adminCopy = (cmsCertificateSettings_().settings || {}).adminEmail || '';
  MailApp.sendEmail({
    to: email,
    bcc: adminCopy || undefined,
    subject: 'Your SK Sapilang Digital Certificate - ' + r[3],
    htmlBody: '<p>Good day, <strong>' + cmsEscapeHtml_(r[2]) + '</strong>.</p>' +
      '<p>Your SK Sapilang digital certificate is available here:</p>' +
      '<p><a href="' + url + '">View / Print Certificate</a></p>' +
      '<p>Certificate No.: <strong>' + id + '</strong></p>'
  });
  sh.getRange(i + 1, 12).setValue('SENT / RESENT ' + new Date());
  return { success: true };
}
