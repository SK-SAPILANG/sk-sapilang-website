...
function cmsCertificateSettings_(p,isPublic){const profile=cmsCertProfile_(p&&p.profile),v=cmsCertificateSettingsSheet_().getDataRange().getDisplayValues().slice(1),all={};v.forEach(r=>all[r[0]]=r[1]);const x=cmsCertDefaults_(profile),keys=['title','body','logo1','logo2','signatory1','position1','design','signatureData','profileLabel'];keys.forEach(k=>{if(all[profile+'.'+k]!==undefined&&all[profile+'.'+k]!=='')x[k]=all[profile+'.'+k];else if(profile!=='default'&&all['default.'+k]!==undefined&&all['default.'+k]!=='')x[k]=all['default.'+k];else if(all[k]!==undefined&&all[k]!=='')x[k]=all[k]});if(!isPublic)x.adminEmail=all.adminEmail||'';return {success:true,profile:profile,settings:x};}
function cmsSaveCertificateSettings_(p){const sh=cmsCertificateSettingsSheet_(),profile=cmsCertProfile_(p.profile),keys=['title','body','logo1','logo2','signatory1','position1','design','signatureData','profileLabel'],vals=sh.getDataRange().getDisplayValues();function set(k,v){let i=vals.findIndex((r,n)=>n>0&&r[0]===k);if(i>0)sh.getRange(i+1,2).setValue(String(v||''));else{sh.appendRow([k,String(v||'')]);vals.push([k,String(v||'')])}}keys.forEach(k=>set(profile+'.'+k,p[k]));if(p.adminEmail!==undefined)set('adminEmail',p.adminEmail);return {success:true,profile:profile};}
