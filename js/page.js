  ...
  window.downloadECertificatesZip = async (btn) => {
    ...
    try {
      ...
      for(let i=0;i<rows.length;i++){
        ...
        // Keep ZIP entries intentionally short for Windows extraction limits.
        // Full participant/activity details remain inside the certificate PDF.
        const shortName = safeFileName(rows[i].participantName || `Certificate-${i+1}`).slice(0, 55) || `Certificate-${i+1}`;
        let entryName = `${shortName}.pdf`;
        let duplicate = 2;
        while (zip.file(entryName)) {
          entryName = `${shortName.slice(0, 48)}-${duplicate++}.pdf`;
        }
        zip.file(entryName,blob);
      }
      const out=await zip.generateAsync({type:'blob'});
      const a=document.createElement('a'); a.href=URL.createObjectURL(out); a.download='E-Certs.zip'; a.click(); setTimeout(()=>URL.revokeObjectURL(a.href),3000);
