...
const CT_DEFAULT_LAYOUT={name:{x:50,y:49,w:62,h:9,fontSize:34},qr:{x:83,y:78,w:11,h:16,fontSize:11},no:{x:18,y:91,w:28,h:5,fontSize:12}};
...
function ctApplyLayout(){
 ...
 Object.entries(CT_LAYOUT).forEach(([k,v])=>{const el=ed.querySelector(`[data-key="${k}"]`);if(!el)return;el.style.left=v.x+'%';el.style.top=v.y+'%';el.style.width=v.w+'%';el.style.height=v.h+'%';el.style.fontSize=(v.fontSize||12)+'px'});
 const bg=document.getElementById('ctBg')?.value||'';if(bg)ed.style.backgroundImage=`url("${bg.replaceAll('"','%22')}")`;
 const h=document.getElementById('ctPreviewHeading');if(h)h.textContent=document.getElementById('ctHeading')?.value||'Certificate';
}
function ctSelectItem(el){document.querySelectorAll('#ctEditor .ct-edit-item').forEach(x=>x.classList.remove('ct-selected'));CT_SELECTED=el||null;if(el)el.classList.add('ct-selected');const n=document.getElementById('ctSelectedItem');if(n)n.textContent=el?`Editing ${el.dataset.key==='name'?'PARTICIPANT NAME':el.dataset.key==='qr'?'QR CODE':'CERTIFICATE NUMBER'} — drag to move; drag orange corner to resize.`:'Select NAME, QR CODE, or CERTIFICATE NO. to move or resize.'}
...
document.getElementById('ctHeading')?.addEventListener('input',ctApplyLayout);
