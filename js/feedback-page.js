...
const CT_DEFAULT_LAYOUT={
 header:{x:50,y:10,w:58,h:5,fontSize:15},logo1:{x:15,y:13,w:9,h:13,fontSize:10},logo2:{x:85,y:13,w:9,h:13,fontSize:10},
 heading:{x:50,y:30,w:60,h:8,fontSize:30},name:{x:50,y:46,w:62,h:9,fontSize:34},citation:{x:50,y:59,w:66,h:18,fontSize:13},
 signature:{x:50,y:79,w:28,h:12,fontSize:12},qr:{x:84,y:82,w:11,h:16,fontSize:11},no:{x:18,y:92,w:28,h:5,fontSize:12}
};
let CT_LAYOUT=JSON.parse(JSON.stringify(CT_DEFAULT_LAYOUT));
let CT_SELECTED=null;
const ctLabel=k=>({header:'HEADER',logo1:'SK LOGO',logo2:'BARANGAY LOGO',heading:'CERTIFICATE TITLE',name:'RECIPIENT NAME',citation:'CITATION / CONTENT',signature:'SIGNATURE BLOCK',qr:'QR CODE',no:'CERTIFICATE NUMBER'}[k]||k.toUpperCase());
function ctApplyLayout(){
 const ed=document.getElementById('ctEditor');if(!ed)return;
 Object.entries(CT_LAYOUT).forEach(([k,v])=>{const el=ed.querySelector(`[data-key="${k}"]`);if(!el)return;el.style.left=v.x+'%';el.style.top=v.y+'%';el.style.width=v.w+'%';el.style.height=v.h+'%';el.style.fontSize=(v.fontSize||12)+'px';el.style.textAlign='center';el.style.fontWeight=['header','heading','name','no'].includes(k)?'700':'400';el.style.fontFamily=['heading','name'].includes(k)?'Georgia,serif':'Arial,sans-serif';});
 const bg=document.getElementById('ctBg')?.value||'';if(bg)ed.style.backgroundImage=`url("${bg.replaceAll('"','%22')}")`;
 const setText=(k,t)=>{const e=ed.querySelector(`[data-key="${k}"]`);if(e){const h=e.querySelector('.ct-resize');e.childNodes[0].nodeValue=t||'';if(h&&!h.parentNode) e.appendChild(h)}};
 setText('header',document.getElementById('ctHeaderText')?.value||'');setText('heading',document.getElementById('ctHeading')?.value||'Certificate');setText('citation',document.getElementById('ctCitation')?.value||'Certificate citation / content appears here.');
 const l1=ed.querySelector('[data-key="logo1"] img'),l2=ed.querySelector('[data-key="logo2"] img');if(l1)l1.src=document.getElementById('ctLogo1')?.value||'';if(l2)l2.src=document.getElementById('ctLogo2')?.value||'';
 const sn=ed.querySelector('.ct-sign-name'),sp=ed.querySelector('.ct-sign-pos'),si=ed.querySelector('[data-key="signature"] img');if(sn)sn.textContent=document.getElementById('ctSignatory')?.value||'';if(sp)sp.textContent=document.getElementById('ctSignatoryPosition')?.value||'';if(si){const u=document.getElementById('ctSignatureUrl')?.value||'';si.src=u;si.style.display=u?'block':'none'}
}
function ctSelectItem(el){document.querySelectorAll('#ctEditor .ct-edit-item').forEach(x=>x.classList.remove('ct-selected'));CT_SELECTED=el||null;if(el)el.classList.add('ct-selected');const n=document.getElementById('ctSelectedItem');if(n)n.textContent=el?`Editing ${ctLabel(el.dataset.key)} — drag to move; drag the orange corner to resize.`:'Select any certificate element above, then drag or resize it.'}
function ctInitVisualEditor(){
 const ed=document.getElementById('ctEditor');if(!ed||ed.dataset.ready)return;ed.dataset.ready='1';
 ed.querySelectorAll('.ct-edit-item').forEach(el=>{
   el.addEventListener('pointerdown',e=>{if(e.target.classList.contains('ct-resize'))return;ctSelectItem(el);e.preventDefault();const r=ed.getBoundingClientRect(),startX=e.clientX,startY=e.clientY,v=CT_LAYOUT[el.dataset.key],sx=v.x,sy=v.y;el.setPointerCapture(e.pointerId);const move=ev=>{v.x=Math.max(0,Math.min(100,sx+(ev.clientX-startX)/r.width*100));v.y=Math.max(0,Math.min(100,sy+(ev.clientY-startY)/r.height*100));ctApplyLayout()};const up=()=>{el.removeEventListener('pointermove',move);el.removeEventListener('pointerup',up)};el.addEventListener('pointermove',move);el.addEventListener('pointerup',up)});
   const h=el.querySelector('.ct-resize');h?.addEventListener('pointerdown',e=>{ctSelectItem(el);e.stopPropagation();e.preventDefault();const r=ed.getBoundingClientRect(),v=CT_LAYOUT[el.dataset.key],startX=e.clientX,startY=e.clientY,sw=v.w,sh=v.h,sf=v.fontSize||12;h.setPointerCapture(e.pointerId);const move=ev=>{const dx=(ev.clientX-startX)/r.width*100,dy=(ev.clientY-startY)/r.height*100;v.w=Math.max(4,Math.min(90,sw+dx*2));v.h=Math.max(3,Math.min(70,sh+dy*2));if(!['qr','logo1','logo2'].includes(el.dataset.key))v.fontSize=Math.max(8,Math.min(80,sf+dx*r.width/100*.7));ctApplyLayout()};const up=()=>{h.removeEventListener('pointermove',move);h.removeEventListener('pointerup',up)};h.addEventListener('pointermove',move);h.addEventListener('pointerup',up)});
 });
 ed.addEventListener('pointerdown',e=>{if(e.target===ed)ctSelectItem(null)});ctApplyLayout();
}
